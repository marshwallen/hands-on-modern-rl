...
初始表全为 0，学习率 $\alpha=0.5$，并采用每次访问都更新的 MC，所以本例的计算模板就是

$$
\text{新值}=\text{旧值}+0.5(\text{MC 目标}-\text{旧值}).
$$

$\text{MC 目标}$ 就是这一行对应的完整回报 $G_t$。比如第 1 次访问 $S$ 时，后面实际拿到 $-2,-1,-2,-1,-1$，所以 $G_t=-7$；第 2 次访问 $S$ 时，前面的 $-2$ 已经过去，不再算入这次访问的未来，所以目标变成 $-5$。
...
这两张表也暴露了 MC 的节奏：**它的目标依赖“从这次访问开始直到终点”的全部奖励**。智能体第一次在 $S$ 撞墙时，只看到了眼前的 $-2$，还不知道后面会不会从 $M$ 退回 $S$，也不知道最后还要走几步。也就是说，此时 $G_t$ 还没有被完整观察到，表里的 $S$ 没有可用的 MC 目标。只有 episode 结束后，整串奖励都确定了，才能从后往前把每次访问的 $G_t$ 算出来，再回头更新访问过的状态。
...
第一段 $R_{t+1}$ 是刚走完一步就能看到的真实奖励；第二段 $G_{t+1}$ 是从下一状态开始一直到终点的完整未来。MC 会等到这段未来真的发生完，再拿到 $G_{t+1}$。TD 的做法更早：既然 $G_{t+1}$ 还没发生完，就先用表里对下一状态的当前估计 $V(S_{t+1})$ 代替它：
...
这一步不是把未来删掉，而是把“未来到底值多少”暂时交给下一状态的价值表来估计。等以后访问下一状态更多次，$V(S_{t+1})$ 自己会继续被修正；前一个状态的估计也会跟着被修正。
...
它度量了旧预测和一步目标之间差多少。可以把它读成一句话：**我原来预测状态 $s$ 值 $V(s)$，但走一步后发现“眼前奖励 + 下一状态估计”是 $r+\gamma V(s')$，两者差了 $\delta$。**如果 $\delta>0$，这次一步目标比旧预测高，表格值会被往上拉；如果 $\delta<0$，一步目标比旧预测低，表格值会被往下压；如果 $\delta=0$，这一次采样下旧预测和一步目标一致。
...
| ---- | ------------ | ------ | ------ | ---- |
| 初始 | 还没开始 | 0 | 0 | 表里还没有经验。 |
| 1 | $S\xrightarrow{-2}S$ | -1 | 0 | $S$ 撞墙，先被眼前的 $-2$ 往下拉。 |
| 2 | $S\xrightarrow{-1}M$ | -1 | 0 | 右走到 $M$，但 $V(M)$ 还是 0，所以这一步没有继续改变 $S$。 |
| 3 | $M\xrightarrow{-2}S$ | -1 | -1.5 | $M$ 左走回到已经变低的 $S$，所以 $M$ 被明显拉低。 |
| 4 | $S\xrightarrow{-1}M$ | -1.75 | -1.5 | $S$ 右走接上较低的 $V(M)$，于是 $S$ 继续下降。 |
| 5 | $M\xrightarrow{-1}G$ | -1.75 | -1.25 | $M$ 右走到终点，只剩一步代价，反而把 $M$ 往上拉一点。 |

这张表和 MC 的表放在一起看，差别就很直观。MC 要等到最后，先算“从这里到终点一共拿了多少”；TD 每走一步就问“这一步奖励是多少，下一状态表里现在写着多少”。它不需要完整未来，只把刚发生的一步和下一格的当前估计接起来。

拿第 1 步对比就很清楚。同样是 $S\xrightarrow{-2}S$，MC 最后给这次访问的目标是 $-7$，因为它看到了后面整局的奖励；TD 当场只能给出 $-2+V(S)=-2$。TD 的目标更粗糙，但它来得早。随着后续一步步更新，后面状态的估计会被写回表里，再通过新的 TD 目标传回前面的状态。
...
DP、MC 和 TD 不是三套互不相干的公式。更好的读法是：**先有一个最 general 的贝尔曼目标，然后在信息越来越少时，target 一步步退化成三种算法各自能用的形式。**
...
它说的是：从状态 $s$ 出发，继续按照当前策略 $\pi$ 行动，未来完整回报 $G_t$ 的平均值是多少。接着把完整回报拆成“眼前一步”和“后面的未来”：
...
到这里还没有 DP、MC、TD，只有一个统一问题：要更新 $V(s)$，target 应该估计右边这个期望。

再往前走一步。到了下一状态 $S_{t+1}$ 之后，如果仍然按照同一个策略 $\pi$ 行动，那么“从下一状态开始的平均未来回报”就是 $V^\pi(S_{t+1})$。所以可以把 $G_{t+1}$ 的期望写成下一状态价值：
...
这就是贝尔曼期望方程最核心的一步：**当前状态的价值，等于“一步奖励 + 下一状态价值”的平均。**

如果把这个期望完全展开，会得到最完整的贝尔曼 target：

$$
\text{target}_{\text{full}}(s)
=
\sum_a
\underbrace{\pi(a\mid s)}_{\text{策略：动作概率}}
\sum_{s',r}
\underbrace{p(s',r\mid s,a)}_{\text{环境模型：下一状态和奖励的分布}}
\left[
\underbrace{r}_{\text{一步奖励}}
+\gamma
\underbrace{V_k(s')}_{\text{旧表里的后续价值}}
\right].
$$
...
| 组成部分 | 含义 |
| -------- | ---- |
| $\sum_a\pi(a\mid s)$ | 按当前策略，对可能动作求平均。 |
| $\sum_{s',r}p(s',r\mid s,a)$ | 按环境模型，对可能下一状态和奖励求平均。 |
| $r+\gamma V_k(s')$ | 每条分支上的“一步奖励 + 后续价值”。 |

::: info 统一 target：先有这一句，再分 DP、MC、TD
要更新 $V(s)$，本质上都在估计
$$
\mathbb{E}_\pi[r+\gamma V(s')\mid s].
$$
区别只是这个期望怎么拿到：DP 把模型展开来算，MC 等完整轨迹后用真实回报替代，TD 只用实际发生的一步再接上旧表估计。
:::

现在才开始分方法。

**第一种：模型已知，完整展开期望，就是 DP。**

如果知道环境模型，就知道每个动作会带来哪些下一状态和奖励。上面的完整 target 可以直接计算。为了和前面走廊例子保持一致，把奖励分布写成平均奖励 $R(s,a)$，把下一状态分布写成 $P(s'\mid s,a)$，就得到 DP 的 target：
...
::: info DP target
**知道 $R$ 和 $P$，所以把完整贝尔曼期望直接展开计算。**

DP 的 target 是模型平均：
$$
\text{target}_{\mathrm{DP}}
=
\text{所有可能分支的加权平均}.
$$
它不需要实际采样，因为 $R(s,a)$ 和 $P(s'\mid s,a)$ 已经告诉了算法所有分支的概率和奖励。
:::

**第二种：不知道模型，不能展开期望，就退化成 MC。**

如果不知道 $R$ 和 $P$，上面的 DP target 立刻卡住：

$$
\boxed{
\text{不知道 } R,P
\quad\Longrightarrow\quad
\text{不能枚举所有分支，也不能算模型平均}
}
$$
...
既然算不出这个期望，就用实际采样来估计它。一次 episode 结束后，从某次访问开始的完整回报 $G_t$ 就是这个期望的一个样本：

$$
\text{target}_{\mathrm{MC}}=G_t.
$$

这里的 $G_t$ 不是模型算出来的平均值，而是这条 episode 里从时刻 $t$ 开始实际拿到的完整回报：
...
::: info MC target
**不知道 $R$ 和 $P$，所以不再展开贝尔曼期望，而是用完整真实回报 $G_t$ 当样本 target。**

MC 去掉的是“模型平均”，保留的是“完整未来”。它必须等到 episode 结束，因为只有那时 $G_t$ 才完整。
:::

**第三种：不等完整未来，只用一步信息，就退化成 TD。**

MC 已经不需要模型了，但它还要等完整 $G_t$。如果连完整回报也不等，就继续从回报递推式退化：

$$
G_t=r_{t+1}+\gamma G_{t+1}.
$$

走完一步后，$r_{t+1}$ 和下一状态 $S_{t+1}$ 已经知道；但 $G_{t+1}$ 还没发生完。于是 TD 做第二个替换：

$$
\boxed{
\text{不知道完整 } G_{t+1}
\quad\Longrightarrow\quad
\text{用旧表里的 } V_k(S_{t+1}) \text{ 先估计后面的未来}
}
$$
...
$$
\text{target}_{\mathrm{TD}}
=r_{t+1}+\gamma V_k(S_{t+1}).
$$

::: info TD target
**不等完整 $G_t$，所以把未知的后续回报 $G_{t+1}$ 换成旧表里的 $V_k(S_{t+1})$。**

TD 保留真实发生的一步：真实奖励 $r_{t+1}$、真实下一状态 $S_{t+1}$。它去掉的是“等待完整未来”，因此每走一步就能更新。
:::

把这条退化链压缩成一张表，就是：

| 层级 | 从哪个 target 出发 | 缺少什么 | 所以怎么替代 | 得到 |
| ---- | ------------------ | -------- | ------------ | ---- |
| 贝尔曼完整 target | $\mathbb{E}_\pi[r+\gamma V(s')\mid s]$ | 什么都不少 | 完整展开动作、奖励、下一状态的期望 | 理论上的统一目标 |
| DP | $\sum_a\pi(a\mid s)\sum_{s',r}p(s',r\mid s,a)[r+\gamma V_k(s')]$ | 不缺模型 | 直接用 $R,P$ 计算期望 | $\text{target}_{\mathrm{DP}}$ |
...
$$
\text{贝尔曼完整期望}
\xrightarrow{\text{知道模型}}
\text{DP：展开期望}
\xrightarrow{\text{不知道 } R,P}
\text{MC：用完整轨迹采样}
\xrightarrow{\text{不等完整未来}}
\text{TD：用一步自举}
$$

注意这里的箭头不是说 DP 先运行、再运行 MC、再运行 TD；它表示 target 的信息条件越来越弱。知道完整模型时，可以用 DP 精确算期望；不知道模型时，只能用 MC 从轨迹中估计；如果连完整轨迹也不想等，就用 TD 把已经发生的一步和旧价值表接起来。
...
可以用一个最小程序把这件事验证一遍。下面的代码使用同一个走廊、同一套策略：在 $S$ 和 $M$ 都以 0.8 的概率右走、0.2 的概率左走。DP 扫表 1000 轮；MC 和 TD 每次各采样 100 万条 episode，并用 5 个不同随机种子重复运行。三种方法的更新方式不同，但估计的是同一个 $V^\pi$。
...
先看单次运行。DP 没有采样随机性；MC 和 TD 用同一个规模的采样，也已经贴近 DP 的值：
...
DP 的输出是模型迭代后的确定结果；MC 和 TD 的输出来自采样，所以单次运行会有小波动，多次运行也会显示一个范围。采样次数足够多时，它们的单次结果、平均结果都会靠近同一组策略价值。这就是这里说的“殊途同归”：三种算法手里拿到的信息不同，更新节奏不同，但在固定策略下，目标都是同一个 $V^\pi$。
